 ...
 * - url: public page is `jobs.smartrecruiters.com/<companyId>/<id>-<slug>`; `j.ref`
 *   only supplies the company id (its /postings/ path shape is NOT a public page).
...
export function parseSmartRecruitersResponse(json, companyName) {
  ...
  return items.map(j => {
    ...
    // Public posting page is jobs.smartrecruiters.com/{CompanyId}/{id}-{slug}.
    // NOTE: .../{CompanyId}/postings/{id} (the ref path shape) is NOT a public page —
    // it 404s in a browser (burned us on 2026-06-11). Use ref only for the company id.
    let companyId = '';
    if (typeof j.ref === 'string') {
      ...
      if (parsedRef
        ...
        companyId = parsedRef.pathname.slice('/v1/companies/'.length).split('/')[0];
      }
    }
    if (!companyId) {
    ...
    let url = '';
    if (companyId && j.id) {
      url = `https://jobs.smartrecruiters.com/${companyId}/${j.id}${slugified ? `-${slugified}` : ''}`;
    }
